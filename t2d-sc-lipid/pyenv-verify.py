...
Check 2 covers the whole lock rather than the fourteen pins written by hand, and
...
Then it writes /opt/pyenv-manifest.tsv, which lists the whole environment --
...
REQ_IN = "/opt/requirements-py.in"
LOCK = "/opt/requirements-py.txt"
MANIFEST = "/opt/pyenv-manifest.tsv"
...
MODULE = {"scikit-learn": "sklearn", "PyYAML": "yaml"}
...
def main():
    ...
    with open(MANIFEST, "w") as fh:
        fh.write(
            ...
            "by /opt/pyenv-verify.py\n"
            f"# python: {sys.version.split()[0]} ({sys.executable})\n"
            f"# pip: {md.version('pip')}\n"
            f"# lock: /opt/requirements-py.txt sha256:{lock_sha}\n"
            "package\tversion\n"
        )
